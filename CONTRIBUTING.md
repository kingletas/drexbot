...
1. **It asks the store for nothing but HTTPS.** No database, no credential, no shell into the stack. It never opens a database connection, and it says so about itself. A check that needs privileged access says what it needs and reports `unsupported` until it has it — it does not reach for a connection string in a config file.
...
3. **A failure names something the operator can act on.** The store may be one whose configuration is not yours to fix, so a failing check carries the store's own error banner and what is actually visible on the page, not only the candidates it tried.
...
- **A check has eight things it can say, not two**, and the eight belong to the kernel. One that cannot mean anything against this store reports `unsupported` and says what it is missing. It never fails, and it never quietly disappears from the sign-off sheet.
