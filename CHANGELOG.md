...
**A browser that cannot start is a setup problem, and says which.** When Chromium couldn't load a system library, every browser check failed as `transport`, was retried as if the network might recover, and carried a page of browser logs. It is now `blocked`, is never retried, folds into one line in the report, and names the fix: _Chromium cannot start because this machine is missing a system library (libnspr4.so) it needs. On Linux, run `make browser-deps`…_. A Chromium that was never installed says to run `make setup`. Any other launch failure is still `transport`.
