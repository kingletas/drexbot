...
**A store Node won't trust says so.** Preflight against a store on a self-signed certificate, or on one whose root this machine hasn't been told about, used to block with only _fetch failed_. It now names the certificate error and the fix: _GET …/magento_version: fetch failed (DEPTH_ZERO_SELF_SIGNED_CERT: self-signed certificate). Node ignores the certificates this machine trusts, so set NODE_EXTRA_CA_CERTS…_. The hint appears only for errors that trusting a root fixes. An expired certificate, a hostname mismatch or a refused connection is named without it. This needs the harness-kernel change that keeps the cause of an HTTP failure, which drexbot now pins.

**The wrapper finds a Warden or Den root.** Both sign each local site's certificate with a root they create on install, and trust it system-wide. Node ignores that trust, so a Warden store failed preflight while the browser accepted it. The wrapper now looks for `~/.warden/ssl/rootca/certs/ca.cert.pem` and the Den equivalent, following `WARDEN_HOME_DIR` and `DEN_HOME_DIR`, alongside the mkcert roots it already found. Node reads only one extra certificate file, so when more than one root is found they are joined into `$XDG_CACHE_HOME/drexbot/extra-ca.pem`. A `NODE_EXTRA_CA_CERTS` you set yourself is still left alone.
