	...
	return [
		{
			...
			async body({ record }) {
				...
				await surface.visit(store.categoryPath, async session => {
					...
					record('first panel', `${before ? 'shown' : 'hidden'} then ${after ? 'shown' : 'hidden'}`)
					if (before === after) {
		...
		},
		{
			...
			async body({ record }) {
				...
				await surface.visit(store.categoryPath, async session => {
					...
					if (count < 2) {
						throw new AssertionFailure(`the comparison holds ${count} product(s) after adding two`)
					}
				})
			},
		},
		{
			...
			async body({ record }) {
				...
				await surface.visit(`/catalogsearch/result/?q=${nonsense}`, async session => {
